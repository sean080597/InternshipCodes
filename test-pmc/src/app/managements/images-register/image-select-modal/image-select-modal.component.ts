...
export class ImageSelectModalComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  openNewImageModal() {
    // if Playlist form/Receipt form else Image Register List
    if (this.typeIdOnly) {
      const initialState = { dragDropFile: this.dragDropFiles[0] }
      this.imageService.openNewRegistModal({ ...initialState, typeIdOnly: this.typeIdOnly }, (res) => {
        if (res.createdImage) {
          this.bsModalRef.content.onClose.next({ key: 'openNewImageModal', createdImage: res.createdImage })
          this.bsModalRef.hide()
        }
      })
    } else {
